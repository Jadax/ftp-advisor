...
- Player Detail Advisor (`player.htm` — keep/release verdict for any single player, squad-peer comparison for 21+, training-potential grid for youth) — **unverified against live markup, see gotcha**: `scrapePlayerDetailPage()`, `comparePlayerToSquadPeers()`, `buildTrainingPotentialGrid()`, `updatePlayerAdvisor()`

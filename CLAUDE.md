...
- Training weekly-gain estimate (real data from the same workbook's "Base Level Training" table, combined with the pre-existing `ACADEMY_SPEED`/age/talent/slowdown multipliers) — **live**, shown as "~Nwk to next level" on Training page recs: `TRAINING_BASE_RATES`, `estimateWeeklyTrainingGain()`, `weeksToNextLevel()`
...
- **Unresolved conflict, don't silently pick a side**: the already-shipped `ACADEMY_SPEED` array (`[0.55...1.5]`, deluxe = 2.7× minimal, also displayed to the user as the Academy page's own "Speed %" column) disagrees with `FTP_Training 5.2.xlsx`'s own academy-multiplier lookup table (`DB!AK598:AL608`, keyed by exact academy name, clean linear `1 + 0.03×level`, deluxe = 1.3× minimal — formula-verified, not a guess). `estimateWeeklyTrainingGain()` currently uses `ACADEMY_SPEED` (internal consistency with the rest of the Academy feature set), not the workbook's curve. Needs the user to confirm against their own live academy page's displayed speed % before either side is trusted further.
...
- `FTP_Training 5.2.xlsx` (`G:\My Drive\FTP_Training 5.2.xlsx`) has a "Player 1/2/3" week-by-week training *simulator* far beyond what's in the script: ~90 columns × 15 weeks × 3 age-blocks per player, driven by a hidden `DB` lookup grid roughly 190 columns × 10 rows of precomputed training-gain values keyed by academy×talent×training-program. The **base weekly rates** (`Refs!I26:Q37`, minimal-academy/100%/no-talent baseline) are already extracted and live in the script as `TRAINING_BASE_RATES` — don't re-derive those. What's NOT yet done: the full `DB` grid's exact per-academy-level and per-talent multiplier matrix (the script instead reuses the already-existing, already-shipped `ACADEMY_SPEED`/`TRAINING_TALENT_BONUS`/`getSkillSlowdownMultiplier` constants, rescaled against `ACADEMY_SPEED[0]` as the reference point — this is a deliberate choice to reuse tested constants over a half-verified new derivation from `DB`, not an oversight) and the full multi-week/multi-year forward planner UI the "Player 1/2/3" tabs actually are. Don't attempt a blind one-pass port of the rest — validate against the sheet's own cached values first (`data_only=True`/raw XML `<v>` cells), since wrong training predictions cost the user real in-game weeks.

...
- Training weekly-gain estimate + multi-week planner (real data from the same workbook's "Base Level Training" table, combined with the pre-existing `ACADEMY_SPEED`/age/talent/slowdown multipliers) — **live**, shown as "~Nwk to next level" + a "📅 12wk outlook" line on Training page recs: `TRAINING_BASE_RATES`, `estimateWeeklyTrainingGain()`, `weeksToNextLevel()`, `simulateTrainingPlan()`, `formatTrainingOutlook()`. The planner re-derives weekly gain every simulated week (skill-slowdown changes as level crosses a threshold) — it does NOT read the workbook's own precomputed `DB!$DB$6:$GL$15` grid, so treat its output as a formula-based estimate consistent with the rest of the script, not a byte-identical reproduction of the spreadsheet.
- Player Detail Advisor (`player.htm` — keep/release verdict for any single player, squad-peer comparison for 21+) — **unverified against live markup, see gotcha**: `scrapePlayerDetailPage()`, `comparePlayerToSquadPeers()`, `updatePlayerAdvisor()`
...
- `scrapePlayerDetailPage()` (player.htm) was written by extending `fetchPlayerPageDetails()`'s already-proven th/td label scan to the rest of the skill grid, **without** a live HTML sample of player.htm itself to confirm against — lower confidence than everything else in this file. It self-checks (`hasFullSkills`, needs 5+ matched skill labels) and shows a warning instead of a false verdict if scraping comes back empty. If the user reports it's not working, ask for the page's HTML before guessing at new selectors.
